  	...
  	<?php require_once "cabecalho.php";?>
		...
		<section>
			...
			if (isset($_POST) && !empty($_POST)){
				...
				$cidade = preg_replace("/[^A-z Á-ú]/","",filter_input(INPUT_POST,'cidade',FILTER_SANITIZE_SPECIAL_CHARS));
				...
				if (!empty($erro)) {
					...
					$resultadoCpf = verificarPaciente($conexao,$cpf);
					
					if($resultadoCpf > 0 ){
						$_SESSION['danger'] = 'esse CPF ja existe';
						header("Location:paciente-form.php");
					}
					else{
						
						
						inserirPaciente($conexao, $cpf, $nome, $telefone, $email, $cep, $cidade, $estado,$bairro, $rua, $numero,$complemento);
